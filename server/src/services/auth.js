...
export const loginService = ({ phone, password }) => new Promise(async (resolve, reject) => {
    try {
        ...
        const token = isCorrectPassword && jwt.sign({ id: user.id, phone: user.phone, role: user.role }, process.env.SECRET_KEY, { expiresIn: '2d' });
        resolve({
            err: token ? 0 : 2,
            msg: token ? 'Login is successfully !' : user ? 'Password is wrong !' : 'Phone number not found !',
            token: token || null
        })
    } catch (error) {
        reject(error)
